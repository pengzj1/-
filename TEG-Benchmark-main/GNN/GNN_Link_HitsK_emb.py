...
@torch.no_grad()
def generate_and_save_embeddings(model, x, adj_t, device, save_path, run):
    """
    Generate and save node embeddings using the trained model
    """
    print("Generating node embeddings...")
    model.eval()
    
    # Load the model with best weights (they were already saved to CPU)
    model = model.to(device)
    
    # Process in batches to avoid potential OOM issues
    batch_size = 10000
    num_nodes = x.size(0)
    embeddings = []
    ...
    for i in range(0, num_nodes, batch_size):
        ...
        batch_x = x.to(device)
        batch_adj_t = adj_t.to(device)
        
        # Generate embeddings
        batch_emb = model(batch_x, batch_adj_t)
    ...
    # Save embeddings
    embeddings_path = f"{save_path}/node_embeddings_run{run}.pt"
    torch.save(all_embeddings, embeddings_path)
    print(f"Node embeddings saved to {embeddings_path}")

# Modify the argument parser in main() to add save_path parameter
def main():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="data/CSTAG/Photo/Model/",
        help="Path to save the best model and embeddings",
    )
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                ...
                # Track the best model based on Hits@10
                _, valid_hits, _ = results["Hits@10"]
                if valid_hits > best_valid_hits:
                    best_valid_hits = valid_hits
                    best_model_state = {k: v.cpu() for k, v in model.state_dict().items()}
                    best_predictor_state = {k: v.cpu() for k, v in predictor.state_dict().items()}
        ...
        # Save the best model and generate embeddings
        if best_model_state is not None:
            model_save_path = f"{args.save_path}/best_model_run{run}.pt"
            predictor_save_path = f"{args.save_path}/best_predictor_run{run}.pt"
            
            torch.save(best_model_state, model_save_path)
            torch.save(best_predictor_state, predictor_save_path)
            print(f"Best model from run {run+1} saved to {model_save_path}")
            
            # Generate and save embeddings using the best model
            generate_and_save_embeddings(model, x, adj_t, device, args.save_path, run)
